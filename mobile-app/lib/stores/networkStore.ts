  ...
  persist(
    (set, get) => ({
      ...
      addNetwork: (network: Network) => {
        set((state) => ({
          networks: [
            ...state.networks.filter((n) => n.chainId !== network.chainId),
            network,
          ],
        }));
      },
      ...
      addCustomNetwork: (
        networkData: Omit<Network, "createdAt" | "updatedAt">,
      ) => {
        const timestamp = new Date().toISOString();
      ...
      removeNetwork: (chainId: number) => {
        set((state) => {
          ...
          if (state.activeNetwork?.chainId === chainId) {
            newActiveNetwork =
              updatedNetworks.find((n) => n.chainId === 1) ||
              updatedNetworks[0] ||
              null;
          }
      ...
      updateNetwork: (chainId: number, updates: Partial<Network>) => {
        set((state) => ({
          ...
          activeNetwork:
            state.activeNetwork?.chainId === chainId
              ? {
                  ...state.activeNetwork,
                  ...updates,
      ...
      checkNetworkConnection: async (chainId: number) => {
        ...
        try {
          ...
          console.log(
            `Checking connection to ${network.name} (${network.rpcUrl})`,
          );
      ...
      getConnectedNetworks: () => {
        ...
        return networks.filter(
          (n) => connectionStatus[n.chainId] === "connected",
        );
      },
