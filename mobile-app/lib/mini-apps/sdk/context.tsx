...
import { MiniAppError, MiniAppPermissionError } from "./types";
...
export function MiniAppProvider({ children }: MiniAppProviderProps) {
  ...
  const createSDK = useCallback(
    (miniAppId: string, manifest: MiniAppManifest): MiniAppSDK => {
      ...
      const walletAPI: WalletAPI = {
        ...
        getBalance: async (tokenAddress?: string) => {
          ...
          try {
            const result = await keyManager.getWalletBalance(
              activeWallet.id,
        ...
        signTransaction: async (transaction: UnsignedTransaction) => {
          ...
          try {
            const result = await keyManager.signTransaction(
              activeWallet.id,
              transaction,
            );
            if (!result.success) {
      ...
      const networkAPI: NetworkAPI = {
        ...
        getNetworkById: (chainId: number) => {
          checkPermission(manifest, "network.read", miniAppId);
          return (
            useNetworkStore.getState().getNetworkByChainId(chainId) || null
          );
        },
