  ...
  persist(
    (set, get) => ({
      ...
      addMiniApp: (miniApp: MiniApp) => {
        set((state) => ({
          miniApps: [
            ...state.miniApps.filter((app) => app.id !== miniApp.id),
            miniApp,
          ],
          installedMiniApps: miniApp.isInstalled
            ? [
                ...state.installedMiniApps.filter(
                  (app) => app.id !== miniApp.id,
                ),
                miniApp,
              ]
      ...
      removeMiniApp: (miniAppId: string) => {
        set((state) => ({
          ...
          installedMiniApps: state.installedMiniApps.filter(
            (app) => app.id !== miniAppId,
          ),
          activeMiniApp:
            state.activeMiniApp?.id === miniAppId ? null : state.activeMiniApp,
          miniAppStates: Object.fromEntries(
            Object.entries(state.miniAppStates).filter(
              ([id]) => id !== miniAppId,
            ),
          ),
        }));
      ...
      updateMiniApp: (miniAppId: string, updates: Partial<MiniApp>) => {
        set((state) => ({
          ...
          activeMiniApp:
            state.activeMiniApp?.id === miniAppId
              ? {
                  ...state.activeMiniApp,
                  ...updates,
                  lastUpdated: new Date().toISOString(),
      ...
      installMiniApp: async (miniAppId: string) => {
        ...
        try {
          ...
          set((state) => ({
            installedMiniApps: [
              ...state.installedMiniApps.filter((app) => app.id !== miniAppId),
              {
                ...miniApp,
                isInstalled: true,
                installationOrder: Date.now(),
      ...
      getInstalledMiniApps: () => {
        const { installedMiniApps } = get();
        return [...installedMiniApps].sort(
          (a, b) => (a.installationOrder || 0) - (b.installationOrder || 0),
        );
      },
